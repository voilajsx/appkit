...
function validateEnvironment() {
    ...
    if (cacheSize) {
        ...
        if (isNaN(cacheSizeNum) || cacheSizeNum <= 0) {
            throw new Error(`Invalid VOILA_UTILS_CACHE_SIZE: "${cacheSize}". Must be a positive number.`);
        }
    ...
    if (cacheTTL) {
        ...
        if (isNaN(cacheTTLNum) || cacheTTLNum <= 0) {
            throw new Error(`Invalid VOILA_UTILS_CACHE_TTL: "${cacheTTL}". Must be a positive number (milliseconds).`);
        }
    }
    ...
    if (arrayThreshold) {
        ...
        if (isNaN(thresholdNum) || thresholdNum <= 0) {
            throw new Error(`Invalid VOILA_UTILS_ARRAY_THRESHOLD: "${arrayThreshold}". Must be a positive number.`);
        }
    }
    ...
    if (chunkLimit) {
        ...
        if (isNaN(chunkLimitNum) || chunkLimitNum <= 0) {
            throw new Error(`Invalid VOILA_UTILS_CHUNK_LIMIT: "${chunkLimit}". Must be a positive number.`);
        }
    }
    ...
    if (numberPrecision) {
        ...
        if (isNaN(precisionNum) || precisionNum < 0 || precisionNum > 20) {
            throw new Error(`Invalid VOILA_UTILS_NUMBER_PRECISION: "${numberPrecision}". Must be between 0 and 20.`);
        }
    }
