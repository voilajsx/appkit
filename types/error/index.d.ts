...
export declare const error: {
    ...
    readonly badRequest: (message?: string) => import("./error").AppError;
    readonly unauthorized: (message?: string) => import("./error").AppError;
    readonly forbidden: (message?: string) => import("./error").AppError;
    readonly notFound: (message?: string) => import("./error").AppError;
    readonly conflict: (message?: string) => import("./error").AppError;
    readonly serverError: (message?: string) => import("./error").AppError;
    readonly createError: (statusCode: number, message: string, type?: string) => import("./error").AppError;
    readonly handleErrors: (options?: ErrorHandlerOptions) => import("./error").ExpressErrorHandler;
    readonly asyncRoute: (fn: AsyncRouteHandler) => (req: import("./error").ExpressRequest, res: import("./error").ExpressResponse, next: import("./error").ExpressNextFunction) => void;
    readonly isClientError: (err: AppError) => boolean;
    readonly isServerError: (err: AppError) => boolean;
};
export type { ErrorConfig } from './defaults';
